<template>
  <div>
    <div class="user-nav">
      <router-link :to="'/user-application'" class="active">待处理申请</router-link>
      <router-link :to="'/user-application/doing'">处理中申请</router-link>
    ...
    <div class="user-vip">
      <el-table class="application"
        ...
        <el-table-column
          ...
          label="操作">
          <template scope="scope">
            <el-button type="text" class="div" @click="dealOrder(scope.row.id)">申请处理</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      dealOrder (id){
        this.$ajax.get('/order/prior',{
          params: {
            ordId: id
          }
        }).then(res => {
          if(res.data && res.data === '成功'){
            this.showCartList()
          }else{
