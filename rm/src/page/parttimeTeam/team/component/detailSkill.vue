    ...
    <table class="parttime-table">
      ...
      <template v-if="biyi.lanPair.length>0">
        <tr v-for="(item, index) of biyi.lanPair" :key="item.id">
          ...
          <td class="name">合作单价</td><td style="padding: 0;width: 100px">
            <span v-if="biyi.show">{{item.unitPrice}}元</span>
            <input v-else v-model="biyi.lanPair[index].unitPrice" type="number">
          </td>
          <td style="padding: 0"><el-button v-show="!biyi.show"
    ...
    <p class="sy-bold sy-title" style="overflow: auto;line-height: 32px">设备
      ...
      <template v-else>
        ...
        <el-button type="success"
                   ...
                   style="float: right;margin-left: 10px">{{shebeiBtn.txt}}</el-button>
        <el-button @click="cancelDevice" style="float: right;">取消</el-button>
      </template></p>
    <table class="parttime-table">
      <tr>
        <td class="name">{{shebeiDetail.auditTime?'成为DTP时间':'申请技能时间'}}</td>
    ...
    <p class="sy-bold sy-title" style="overflow: auto;line-height: 32px">搭建
      ...
      <template v-else>
        ...
        <el-button type="success"
                   ...
                   style="float: right;margin-left: 10px">{{dajianBtn.txt}}</el-button>
        ...
        this.$http.get('/userExtension/findResumeByUserId', {
          ...
          if(res.data.message === 'success'){
            ...
            if(_data.userExtension){
              ...
              this.biyi.transYear = parseInt(this.skillDetail.transYear);
              this.biyi.lanPair = [];
            ...
            if(this.huizhanDetail.languageInfo){
              ...
              lanArr.forEach(item => {
                this.getSecondField(item.firstAreaCode).then(res => {
                  const obj = {
                    langaugeName: item.langaugeCode+','+item.langaugeName,
