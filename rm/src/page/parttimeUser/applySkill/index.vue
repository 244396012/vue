          ...
          <div class="grid-content bg-purple dotted-border-rg">
            <el-form :inline="true" class="demo-form-inline filterForm" label-width="90px">
              ...
              <el-form-item label="翻译题测试">
                <el-select v-model="form.trans"
                           placeholder="请选择翻译题测试">
                  <el-option
                    v-for="item in formSelect.passOptions"
                    :key="item"
                    :label="item"
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button type="text" @click="$router.push('/parttimeUser/applySkill/detail/'+scope.row.id)">查看</el-button>
            <el-button type="text"
...
<script>
  ...
  export default {
    ...
    filters: {
      ...
      formatArr (arrStr){
        let idx = arrStr.indexOf(';');
        if(idx > -1){
          const arr = arrStr.slice(0, idx);
          if(arr.includes('[')){
